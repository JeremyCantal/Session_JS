...
class Guerrier{
    ...
    attaquer(cible)
    {
        console.log("Attaquant : " + this.nom + " => [ valAtt = " + this.valAtt + " ] / [ ValDef = " + this.valDef + " ] / [ valLife = " + this.valLife + " ]");
        console.log("Attaqué : " + cible.nom + " => [ valAtt = " + cible.valAtt + " ] / [ valDef = " + cible.valDef + " ] / [ valLife = " + cible.valLife + " ]");
        var deg = Math.round(this.valAtt - (this.valAtt * cible.valDef / 100), 2);

        console.log("L'attaque de " + this.nom + " a fait " + deg + " points de dégâts d'attaque à " + cible.nom + ".");
...
function nouveauGuerrier(){
    console.log("Ajout d'un champion de type guerrier");
    let refChampion = 10;
    perso = new Guerrier(prompt("Choisissez un nom ", ), prompt("Quels seront ses dégâts d'attaque ? ", ), prompt("Combien aura t'il de points de défense ? ", ), prompt("A combien s'élève ses PV ? ", ) );
    alert(perso.getAll());
}
...
class Magicien{
    ...
    attaquer(cible)
    {
        console.log("Attaquant : " + this.nom + " => [ valAtt = " + this.valAtt + " ] / [ ValDef = " + this.valDef + " ] / [ valLife = " + this.valLife + " ] / [ valMana = " + this.valMana + " ]");
        console.log("Attaqué : " + cible.nom + " => [ valAtt = " + cible.valAtt + " ] / [ valDef = " + cible.valDef + " ] / [ valLife = " + cible.valLife + " ]");
        var deg = Math.round(this.valAtt - (this.valAtt * cible.valDef / 100), 2);
        var nouvMana = this.valMana - (100);

            if((this.valAtt / 2) > this.valMana){
                return console.log(this.nom + " n'a pas assez de mana pour attaquer. => Solde : " + this.valMana + ".");
            }
            else{
                console.log("L'attaque de " + this.nom + " a fait " + deg + " points de dégâts d'attaque à " + cible.nom + ".");
                console.log(this.nom + " perd " + (this.valAtt / 2) + " de mana. Nouveau solde de mana : " + nouvMana + ".");
                cible.setValLife(cible.valLife - deg);
                this.setValMana(nouvMana);
                if(cible.valLife <= 0){
                    console.log(this.nom + " a tué " + cible.nom + " :'( ");
                }
            }
